...
function ensureModeStats(obj) {
  ...
  for (const m of modes) {
    if (!obj.modeStats[m]) obj.modeStats[m] = { sent: 0, replied: 0, dates: 0, ghosts: 0 };
    for (const k of ['sent', 'replied', 'dates', 'ghosts']) {
...
function ensureUserSchema(u) {
  ...
  // global counts
  if (!u.stats) u.stats = { sent: 0, replied: 0, dates: 0, ghosts: 0 };
  for (const k of ['sent', 'replied', 'dates', 'ghosts']) if (typeof u.stats[k] !== 'number') u.stats[k] = 0;
  ...
  for (const [, g] of Object.entries(u.girls)) {
    ...
    for (const k of ['sent', 'replied', 'dates', 'ghosts']) if (typeof g.stats[k] !== 'number') g.stats[k] = 0;
...
function getGirl(user, name) {
  ...
  if (!user.girls[key]) {
    user.girls[key] = {
      ...
      history: [],
      notes: [],
      stats: { sent: 0, replied: 0, dates: 0, ghosts: 0 },
...
// --------------------
function updateLearning(user, mode, outcome) {
  const L = user.learning;
  const delta = (k, v) => (L[k] = clamp(L[k] + v, -3, 3));

  const good = outcome === 'replied' ? 1 : outcome === 'date' ? 2 : 0;
  const bad = outcome === 'ghost' ? -1 : 0;
...
function pickMode(user, girl) {
  ...
  const vals = modes.map((m) => {
    ...
    const v = 0.55 * vGlobal + 0.45 * vGirl;
    return { m, v };
  });
...
function combinedKeyboard() {
  return Markup.inlineKeyboard([
    ...
    [Markup.button.callback('💬 Она ответила', 'out_replied'), Markup.button.callback('📅 Встреча', 'out_date')],
...
bot.command('girl', async (ctx) => {
  ...
  saveStore();
  await ctx.reply(`Ок. Активная: ${key}\nКонтекст: ${data.ctx}\nNotes: ${(data.notes || []).length}\nSuccess: ${scoreText(data.conv)}`);
});
...
bot.command('stats', async (ctx) => {
  ...
  await ctx.reply(
    `Стата (общая):\n` +
      ...
      `Адаптация: ${hint.summary}\nИнструкция: ${hint.instruction}\n` +
      `A/B autopick: ${user.settings.autopick ? 'ON' : 'OFF'} | Autoghost: ${user.settings.autoghostHours}h`
  );
});
...
// --------------------
async function autoMarkRepliedIfNeeded(user, girl) {
  const t = girl.thread;
  if (t && !t.closed && t.sentCount > 0) {
    ...
    bumpModeStats(user, mode, 'replied', 0);
    bumpModeStats(girl, mode, 'replied', 0);

    updateLearning(user, mode, 'replied');
    closeThread(user, girl, 'replied');

    return { did: true, mode };
  }
  return { did: false, mode: null };
}
...
bot.on('text', async (ctx) => {
  ...
  const auto = await autoMarkRepliedIfNeeded(user, girl);
  ...
  await ctx.reply(
    `Думаю… (девушка: ${key})` +
      (auto.did ? `\nАвто-метка: 💬 она ответила (mode=${auto.mode})` : '') +
...
async function handleOutcome(ctx, outcome) {
  ...
  updateLearning(user, mode, outcome);
    ...
    for (const [, uRaw] of store.entries()) {
      ...
      for (const [, girl] of Object.entries(user.girls)) {
        ...
        if (t.lastSentAt < cutoff) {
          ...
          updateLearning(user, mode, 'ghost');
